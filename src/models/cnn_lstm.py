...
class CNN_LSTM(Sequential):
  def __init__(self, input_shape=(250,1,22)):
    ...
    # Conv. block 1
    self.add(Conv2D(filters=25, kernel_size=(10,1), padding='same', activation='elu', input_shape=input_shape))
    ...
    self.add(Reshape((100,1))) # Reshape output of FC layer so that it's compatible
